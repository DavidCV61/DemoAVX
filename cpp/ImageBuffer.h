  ...
  void Allocate(size_t height, size_t width, PixelType pixel_type) {
    if (m_PixelBuffer != nullptr)
      throw std::runtime_error(
          "ImageBuffer::Allocate() - invalid value (m_PixelBuffer)");
    ...
    if (pixel_type == PixelType::Gray8)
      ...
      throw std::runtime_error(
          "ImageBuffer::Allocate() - invalid value (pixel_type)");
    ...
    m_PixelBuffer =
        OS::AlignedMalloc(m_PixelBufferSize, c_PixelBufferAlignment);
  };
...
public:
  ...
  template <typename T> T *GetPixelBuffer(void) const {
    return (T *)m_PixelBuffer;
  }
  uintptr_t GetPixelBufferUintptr(void) const {
    return (uintptr_t)m_PixelBuffer;
  }
  ...
  ImageBuffer(ImageBuffer &&ib) noexcept
      : m_Height(ib.m_Height), m_Width(ib.m_Width),
  ...
  static bool IsConforming(const ImageBuffer &ib1, const ImageBuffer &ib2) {
    return ib1.m_PixelType == ib2.m_PixelType && ib1.m_Height == ib2.m_Height &&
           ib1.m_Width == ib2.m_Width;
  }
