...
public:
  ...
  size_t GetPixelBufferSize() const {
    return m_ImageBuffer.GetPixelBufferSize();
  }
  PixelType GetPixelType() const { return m_ImageBuffer.GetPixelType(); }
  template <typename T> T *GetPixelBuffer(void) const {
    return m_ImageBuffer.GetPixelBuffer<T>();
  }
  uintptr_t GetPixelBufferUintptr(void) const {
    return m_ImageBuffer.GetPixelBufferUintptr();
  }

  ImageMatrix(const char *filename, PixelType pixel_type,
              Channel channel = Channel::None) {
    if (pixel_type != PixelType::Gray8 && pixel_type != PixelType::Rgb32)
      throw std::runtime_error(
          "ImageMatrix::LoadImage() - unsupported pixel_type");
  ...
  friend std::ostream &operator<<(std::ostream &os, const ImageMatrix &im) {
    ...
    std::stringstream ss;
    ss << "0x" << std::setfill('0') << std::hex << std::setw(16)
       << im.GetPixelBufferUintptr() << nl;
  ...
  template <typename T> std::vector<T> ToVector(void) {
    if (m_ImageBuffer.GetPixelType() != PixelType::Gray8)
      throw std::runtime_error(
          "ImageMatrix::ToVector() - unsupported pixel_type");
  ...
  template <typename T>
  static ImageMatrix ToImage(std::vector<T> &v_src, size_t im_h, size_t im_w,
  ...
  void FillRandom(uint8_t rng_min, uint8_t rng_max, unsigned int rng_seed) {
    ...
    if (pixel_type == PixelType::Gray8) {
      ...
      throw std::runtime_error(
          "ImageMatrix::FillRandom() - invalid pixel type");
  }

  void AllocRgbArrays(uint8_t **r, uint8_t **g, uint8_t **b,
                      unsigned int alignment) {
    if (m_ImageBuffer.GetPixelType() != PixelType::Rgb32)
      throw std::runtime_error(
          "ImageMatrix::AllocRgbArrays() - invalid pixel type");
  ...
  void FreeRgbArrays(uint8_t *r, uint8_t *g, uint8_t *b) {
    if (m_ImageBuffer.GetPixelType() != PixelType::Rgb32)
      throw std::runtime_error(
          "ImageMatrix::FreeRgbArrays() - invalid pixel type");
  ...
  void GetRgbPixelData(uint8_t *r, uint8_t *g, uint8_t *b) {
    if (m_ImageBuffer.GetPixelType() != PixelType::Rgb32)
      throw std::runtime_error(
          "ImageMatrix::GetRgbPixelData() - invalid pixel type");
  ...
  void SetRgbPixelData(const uint8_t *r, const uint8_t *g, const uint8_t *b) {
    if (m_ImageBuffer.GetPixelType() != PixelType::Rgb32)
      throw std::runtime_error(
          "ImageMatrix::SetRgbPixelData() - invalid pixel type");
  ...
  static bool AreAlike(const ImageMatrix &im1, const ImageMatrix &im2,
                       uint8_t max_delta, size_t *num_diff) {
    *num_diff = 0;
    ...
    throw std::runtime_error(
        "ImageMatrix::AreAlike() - unsupported pixel type");
  }
};
