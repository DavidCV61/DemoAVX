...
class ImageMatrixL {
  static const int c_PngTempBufferSize =
      2048 * 2048 * sizeof(RGB32); // h * w * sizeof(RGB32)

public:
  static void LoadImage(ImageBuffer &ib, const char *filename,
                        PixelType pixel_type, Channel channel) {
    if (!MF::IsExt(filename, "png"))
      throw std::runtime_error(
          "ImageMatrixL::LoadImage() - unsupported image file type");
  ...
  static void SaveImage(ImageBuffer &ib, const char *filename,
                        ImageFileType file_type) {
    if (file_type != ImageFileType::PNG)
      throw std::runtime_error(
          "ImageMatrixL::SaveImage() - unsupported file_type");
    ...
    if (pixel_type == PixelType::Gray8) {
      ...
      throw std::runtime_error(
          "ImageMatrixL::SaveImage() - unsupported pixel_type");
  }
};
