  ...
  void AllocRowPointers(void) {
    if (m_RowPointers != NULL)
      throw std::runtime_error(
          "ImagePng::AllocRowPointers - m_RowPointers not NULL");
  ...
  void FreeRowPointers(bool dtor = false) {
    if (m_RowPointers == NULL) {
      if (dtor)
        ...
        throw std::runtime_error(
            "ImagePng::FreeRowPointers - m_RowPointers is NULL");
    }
...
public:
  ...
  void LoadImage(const std::string &filename, uint8_t *pixel_buff,
                 int pixel_buff_size, int *height, int *width) {
    //
    //
    //
    ...
    if (!m_Png)
      throw std::runtime_error(
          "ImagePng::LoadImage - png_create_read_struct error");

    m_PngInfo = png_create_info_struct(m_Png);

    if (!m_PngInfo)
      throw std::runtime_error(
          "ImagePng::LoadImage - png_create_info_struct error");
  ...
  void SaveImage(const std::string &filename, const uint8_t *pixel_buff,
    ...
    if (!m_Png)
      throw std::runtime_error(
          "ImagePng::SaveImage - png_create_write_struct error");

    m_PngInfo = png_create_info_struct(m_Png);

    if (!m_PngInfo)
      throw std::runtime_error(
          "ImagePng::SaveImage - png_create_info_struct error");
    ...
    png_set_IHDR(m_Png, m_PngInfo, m_Width, m_Height, m_BitDepth, m_ColorType,
