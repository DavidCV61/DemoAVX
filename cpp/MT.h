...
namespace MT {
template <typename T>
void FillArray(T *x, size_t n, int min_val, int max_val, unsigned int rng_seed,
...
template <typename T>
void FillMatrix(T *x, size_t nrows, size_t ncols, int min_val, int max_val,
                unsigned int rng_seed, bool exclude_zero = false) {
  FillArray(x, nrows * ncols, min_val, max_val, rng_seed, exclude_zero);
}

template <typename T>
void FillArrayFP(T *x, size_t n, T min_val, T max_val, unsigned int rng_seed) {
...
template <typename T>
void FillMatrixFP(T *x, size_t nrows, size_t ncols, T min_val, T max_val,
                  unsigned int rng_seed) {
  FillArrayFP(x, nrows * ncols, min_val, max_val, rng_seed);
}

template <typename T>
bool CompareArraysFP(const T *a1, const T *a2, size_t n, T epsilon) {
  for (size_t i = 0; i < n; i++) {
...
template <typename T>
bool CompareVectorsFP(const std::vector<T> &v1, const std::vector<T> &v2,
                      T epsilon) {
  if (v1.size() != v2.size())
    return false;
